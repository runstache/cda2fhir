...
public interface IValidator {
  /**
   * Sets the terminology server of the validator object and make the connection
   * to the given server.
   * 
   * @param termServerUrl A terminology server URL String
   */
  void setTerminologyServer(String termServerUrl);
  ...
  /**
   * Validates a FHIR IResource instance by using the validation engine supplied
   * by hl7.org
   * 
   * @param resource A FHIR IResource instance. If the (DAF) profile is supplied
   *                 in meta.profile attribute, then (DAF) profile validation is
   *                 enable automatically.
   * @return An output stream containing the validation result. The validation
   *         result is contained in div element.
   */
  OutputStream validateResource(Resource resource);
}