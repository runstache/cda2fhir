...
public class CcdTransformerImpl implements ICdaTransformer, Serializable {
  ...
  /**
   * Transforms a CDA Document to a FHIR Bundle.
   * 
  ...
  public Bundle transformDocument(ClinicalDocument cda) {
    ...
    Bundle ccdBundle = resTransformer.transformClinicalDocument2Composition(ccd);
    ...
    // init the patient id reference if it is not given externally. the patient is
    // always the 2nd bundle entry
    if (patientRef == null) {
    ...
    for (Section cdaSec : ccd.getSections()) {
      ...
      if (cdaSec instanceof AdvanceDirectivesSection) {
        ...
        for (ImmunizationActivity immAct : immSec.getImmunizationActivities()) {
          Bundle immBundle = resTransformer.transformImmunizationActivity2Immunization(immAct);
        ...
        for (org.openhealthtools.mdht.uml.cda.Procedure procedure : equipSec.getProcedures()) {
          if (procedure instanceof ProcedureActivityProcedure) {
            Bundle procBundle = resTransformer.transformProcedure2Procedure(procedure);
        ...
        for (MedicationActivity medAct : medSec.getMedicationActivities()) {
          Bundle medBundle = resTransformer.transformMedicationActivity2MedicationStatement(medAct);
        ...
        for (ProblemConcernAct pcAct : probSec.getConsolProblemConcerns()) {
          Bundle conBundle = resTransformer.transformProblemConcernAct2Condition(pcAct);
        ...
        for (ProcedureActivityProcedure proc : procSec.getConsolProcedureActivityProcedures()) {
          Bundle procBundle = resTransformer.transformProcedure2Procedure(proc);
          mergeBundles(procBundle, ccdBundle, fhirSec, Procedure.class);
        }
        ...
        for (ResultOrganizer resOrg : resultSec.getResultOrganizers()) {
          Bundle resBundle = resTransformer.transformResultOrganizer2DiagnosticReport(resOrg);
        ...
        for (org.openhealthtools.mdht.uml.cda.Observation socialObs : socialSec.getObservations()) {
          Bundle socialObsBundle = resTransformer.transformObservation2Observation(socialObs);
        ...
        for (VitalSignsOrganizer vsOrg : vitalSec.getVitalSignsOrganizers()) {
          for (VitalSignObservation vsObs : vsOrg.getVitalSignObservations()) {
            Bundle vsBundle = resTransformer.transformVitalSignObservation2Observation(vsObs);
   ...
   * @param sectionRefCls Specific FHIR Resource Class among the resources in the
   *                      sourceBundle, whose reference will be added to the FHIR
   *                      Section
   */
  private void mergeBundles(Bundle sourceBundle, Bundle targetBundle, SectionComponent fhirSec,
      Class<?> sectionRefCls) {
